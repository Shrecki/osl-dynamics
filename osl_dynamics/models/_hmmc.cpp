...
{
    return a == -std::numeric_limits<double>::infinity()   ? b
    ...
    py::array_t<double> frameprob_)
{
    ...
    if (startprob.shape(0) != nc || transmat.shape(0) != nc || transmat.shape(1) != nc)
    {
    ...
    py::array_t<double> log_frameprob_)
{
    ...
    if (log_startprob.shape(0) != nc || log_transmat.shape(0) != nc || log_transmat.shape(1) != nc)
    {
        throw std::invalid_argument{"shape mismatch"};
    }
    ...
    py::array_t<double> frameprob_)
{
    ...
    auto frameprob = frameprob_.unchecked<2>();
    auto ns = frameprob.shape(0), nc = frameprob.shape(1);
    if (fwd.shape(0) != ns || fwd.shape(1) != nc || transmat.shape(0) != nc || transmat.shape(1) != nc || bwd.shape(0) != ns || bwd.shape(1) != nc)
    {
        throw std::invalid_argument{"shape mismatch"};
    }
    ...
    for (auto t = 0; t < ns - 1; ++t)
    {
        for (auto i = 0; i < nc; ++i)
        {
            for (auto j = 0; j < nc; ++j)
            {
                xi_sum(i, j) += fwd(t, i) * transmat(i, j) * frameprob(t + 1, j) * bwd(t + 1, j);
            }
        }
    }
    return xi_sum_;
